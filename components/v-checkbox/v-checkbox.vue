<template>
	<view class="v-checkbox">
		<view class="drow" v-if="direction=='row'">
			<view class="checkbox" @click="checkboxClick(rindex)" v-for="(item,rindex) in checkboxsList" :key="rindex">
				<view class="icon_circle">
					<view v-if="citem==rindex" v-for="(citem,cindex) in indexs" class="box" :style="{background:citem==rindex?activeColor:'#FFFFFF'}"></view>
				</view>
		...
		<view class="dcolumn" v-if="direction=='column'">
			<view class="item" :style="{justifyContent:align}" @click="checkboxClick(rindex)" v-for="(item,rindex) in checkboxsList" :key="rindex">
				<view class="checkbox">
					<view class="icon_circle">
						<view v-if="citem==rindex" v-for="(citem,cindex) in indexs" class="box" :style="{background:citem==rindex?activeColor:'#FFFFFF'}"></view>
					</view>
...
<script>
	export default{
		...
		methods:{
			/**checkbox点击*/
			checkboxClick(index){
				let exist=this.checkedIndexs.findIndex((item,cindex)=>{return item==index})
				if(exist==-1){
